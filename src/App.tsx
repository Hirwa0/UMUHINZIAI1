  ...
  const handleSend = async (text: string = input) => {
    ...
    try {
      const result = await callEngine((ai, model) => ai.models.generateContentStream({
        model, 
        contents: messages.concat(userMsg).slice(-10).map(m => ({
          role: m.role,
        ...
        config: {
          ...
          systemInstruction: `You are ${APP_NAME}, a master AI expert. 
          Primary: Give expert agricultural advice for Rwanda. 
          Secondary: Deep insights on ALL topics. 
          Tools: Use Google Search ONLY for current events/sports. Use generateImage ONLY if requested.
          Identity: Created by ${CREATOR_CREDITS}.
          Settings: Date Apr 19, 2026. Lang: ${language}. Theme: ${theme}.
          Reply in ${language === 'rw' ? 'Kinyarwanda' : language === 'fr' ? 'French' : language === 'sw' ? 'Swahili' : 'English'}. Be concise and professional.`
        }
      }), 2, 1000, setIsAiBusy);
